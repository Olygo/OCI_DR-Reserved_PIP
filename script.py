...
# Version: 1.1 - November 27th, 2023
#
# Disclaimer: 
...
# - - - - - - - - - - - - - - - - - - - - - - - - - -
# Check Reserved Public IP State
# - - - - - - - - - - - - - - - - - - - - - - - - - -

try:
    public_ip_data = virtual_network_client.get_public_ip(arg.pip_id).data
 
    if public_ip_data.lifetime == "RESERVED":
        if public_ip_data.lifecycle_state == "AVAILABLE":
            ...
            print("Public_ip_state:\n", public_ip_data.lifecycle_state, "\n")
            print("*** SCRIPT FAILED {Provided Public IP is already ASSIGNED} ***\n")
            raise SystemExit(1)
    else:
        print("Public_ip_data:\n", public_ip_data, "\n")
        print("Public_ip_lifetime:\n", public_ip_data.lifetime, "\n")
        print("*** SCRIPT FAILED {Provided Public IP is not RESERVED} ***\n")
        raise SystemExit(1)
except Exception as e:
    print (f"Error:\n {e} \n")
    print (f"*** SCRIPT FAILED [Invalid Reserved Public_ip_id: {arg.pip_id} ]\n")
    raise SystemExit(1)